...
const DotGrid: React.FC<DotGridProps> = ({
  dotSize = 6, // small dots
  gap = 22,    // slightly less gap for denser grid
  baseColor = "#00d8ff",
  activeColor = "#00d8ff",
  proximity = 150,
  speedTrigger = 100,
  ...
  const buildGrid = useCallback((): void => {
    ...
    for (let i = 0; i < total; i++) {
      ...
      dot.style.opacity = "0.13"; // more subtle opacity
    ...
    const onMove = (e: MouseEvent): void => {
      ...
      requestAnimationFrame(() => {
        centresRef.current.forEach(({ el, x, y }) => {
          ...
          gsap.set(el, {
            backgroundColor: interp > 0.2 ? activeColor : baseColor,
            // If the dot is being enlarged (interp > 0.2), use activeColor (blue)
          });
